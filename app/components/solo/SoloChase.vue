...
<template>
  <div class="solo-enc solo-enc--chase" role="alertdialog" aria-modal="true" :style="{ '--glow': glow }">
    <Transition name="fade" mode="out-in">
      <SoloDepth v-if="deep" :key="`d-${chase.art}`" class="solo-enc__art" :src="`/art/${story}/${chase.art}.jpg`" :depth="`/art/${story}/z_${chase.art}.jpg`" mode="none" :grade="[0.5, 1.25, 0.7]" :lx="50" :ly="50" :focus="focus?.[chase.art]" :fog="0.18" motion="run" :wind="wind?.includes(chase.art) ? `/art/${story}/w_${chase.art}.jpg` : undefined" :windy="1.3" :leaves="1.3" @fail="depthFail = true" />
