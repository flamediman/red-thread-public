  ...
  if (windOn > 0.5) {
    ...
    if (max(gmask.x, gmask.y) > 0.08) for (int k = 0; k < 3; k++) {
      ...
      for (int j = 0; j < 4; j++) for (int i = -1; i <= 1; i++) {
        ...
        if (a < 0.01) continue;
        // цвет — с картинки у корня, как раньше (травинка читается своей, не растворяется в фоне); ограничены только
        // слишком светлые: травинка со светлого пятна у корня горела на тёмной траве (25.09 цвет подтягивали к фону —
        // трава почти пропала, её вклад в кадр упал вшестеро). Над краем газона (стена, бордюр) — чуть прозрачнее
        vec3 bc = textureLod(img, ro, 1.0).rgb * (0.75 + 0.35 * sl) * (0.85 + 0.3 * hash(c + 5.5));
        bc = min(bc, under * 1.3 + 0.03);
        a *= 0.8 + 0.2 * smoothstep(0.1, 0.5, gmask.x);
