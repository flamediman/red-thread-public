...
//   node tools/depth-maps.mjs ../red-thread-secret/<история>/art [--force]   — для всех l_*/o_* без готовой z_*; затем имена — в SoloStory.depth
// Ближнее расширено на несколько пикселей за свои края (dilate): иначе край предмета несёт пиксели фона и при параллаксе двоится
...
for (const f of list) {
  ...
  await sharp(Buffer.from(d.data), { raw: { width: d.width, height: d.height, channels: d.channels ?? 1 } }).resize(meta.width, meta.height).dilate(5).blur(1.6).jpeg({ quality: 90 }).toFile(out)
  done++
}
