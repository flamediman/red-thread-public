...
export interface SoloLine {
  ...
  art?: string
  sfx?: string[]
  /** реплика звучит, только если условие выполнено к началу сцены (id озвучки от этого не сдвигаются) */
  when?: SoloCond
}
...
export interface SoloEffect {
  ...
  give?: string[]
  /** найти то, чего не хватает (SoloStory.supply), — наверняка, если нужда есть; когда всего вдоволь — ничего */
  supply?: boolean
  take?: string[]
  set?: string[]
  unset?: string[]
...
export interface SoloHotspot {
  ...
  hideWhen?: SoloCond
  /** тайник: при осмотре может найтись то, чего герою сейчас не хватает (SoloStory.supply) */
  stash?: boolean
}
...
/** Запас по нужде (как в Resident Evil): в тайниках (SoloHotspot.stash) находится то, чего не хватает, — патроны к стволу,
    который есть, лекарство, батарейка. Чем острее нужда, тем вероятнее; когда всего вдоволь — пусто. Кончились патроны
    совсем — в следующем тайнике они точно. Между находками — передышка игрового времени (cooldownMs, по умолчанию 150 с);
    уже обысканный тайник обыскивается ещё раз, только если нужда острая */
export interface SoloSupply {
  /** патроны: ствол → предмет-патроны, что кладётся в тайник */
  ammo: Record<string, string>
  /** лекарство (первое — когда совсем плохо), батарейка */
  heal: string[]
  battery?: string
  /** строка находки по предмету: «Под бумагами — картонная пачка…» */
